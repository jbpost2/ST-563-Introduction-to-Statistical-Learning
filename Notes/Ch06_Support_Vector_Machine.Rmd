...
The support vector machine (SVM) is a family of classification rules that contain both parametric (e.g., linear) and nonparametric (e.g., kernel based) methods. It is often considered as one of the ready-to-use classifiers. It can be viewed as a generalization of linear decision boundaries
for classification. In particular, SVM produces nonlinear boundaries by constructing a linear boundary in a large, transformed version of the feature space. We will mainly discuss three approaches: maximal margin classifier, the support vector classifier, and the support vector machine.^[People often loosely refer to these methods collectively as *support vector machines*.]


# Maximal margin classifier

```{r svm1, echo=F, fig.height=6, fig.width=6, fig.margin=TRUE, fig.cap="Simulated two class data with support vectors and separating hyperplane (line) highlighted."}
...
Let us take a look at the following artificially generated dataset. The dataset contains two predictors, $X_1$ and $X_2$, and observations come from two classes (blue circles and orange triangles); see Figure \ref{fig:svm1}. The two classes are well separated, and a straight line can be used for classification. This situation is called *linearly separable*. Formally, in a linearly separable case, there exists $\beta_0, \beta_1, \beta_2$ such that the line
$$
...
$$
Y_i = -1 \hbox{ if } \beta_0 + X_{i1}\beta_1 + X_{i2}\beta_2 < 0; \;\;  Y_i = +1 \hbox{ otherwise.} 
$$
In other words,
$$
Y_i = sign(\beta_0 + X_{i1}\beta_1 + X_{i2}\beta_2).
$$
In general, with $p$ predictors, $X_{i1}, \ldots, X_{ip}$, we will use a linear combination of the form  $\beta_0 + X_{i1}\beta_1 + \ldots +  X_{ip}\beta_p$. This is called a *hyperplane*, and thus the name *separating hyperplane*. One crucial property of a separating hyperplane is that, 
$$
Y_i(\beta_0 + X_{i1}\beta_1 + \ldots + X_{ip}\beta_p) > 0, \; i = 1, \ldots, n.
$$

When such a separating hyperplane exists, we can construct a natural classifier: for a new observation $\x = (x_1, \ldots, x_p)$, the predicted class is
$$
\widehat Y = sign(\beta_0 + x_{1}\beta_1 + \ldots + x_{p}\beta_p). 
$$
In other words, a test observation is assigned a class depending on which side of the hyperplane it is located. 


One possible was to find such a separating hyperplane is to minimize the distance of misclassified points to the decision boundary. If a response $Y_i = 1$ is misclassified, then $\beta_0 + X_{i1}\beta_1 + \ldots +  X_{ip}\beta_p < 0$, and the opposite for a misclassified response with $Y_i = -1$. Also note that the magnitude of $\beta_0 + X_{i1}\beta_1 + \ldots +  X_{ip}\beta_p$ tells us how far the observation $(X_{i1}, \ldots, X_{ip})$ is located from the boundary. Thus we can minimize
$$
- \sum_{i \in {\cal M}} Y_i(\beta_0 + X_{i1}\beta_1 + \ldots +  X_{ip}\beta_p)
$$
with respect to $\beta_0, \ldots, \beta_p$, where ${\cal M}$ indexes the set of misclassified points. It can be shown that the quantity above is nonnegative and proportional to the distance of the misclassified points to the decision boundary defined by $\beta_0 + X_1\beta_1 + \ldots +  X_p\beta_p = 0$. 

```{r svm2b, echo=FALSE, fig.cap="Examples of multiple separating hyperplanes for the same data."}
plot(all$x1, all$x2, pch=c(19, 17)[all$group], 
     col = c("steelblue", "darkorange")[all$group], cex=2,
     xlab = "X1", ylab = "X2")
...
However, when the problem is linearly separable, there are infinitely many such separating hyperplanes, see Figure \ref{fig:svm2b}. In this perfectly separable case, given any separating hyperplane, we can define the *margin* as the minimal distance from the observations to the hyperplane.^[Formally, for a given separating hyperplane, define $M_i =$ distance between the hyperplane and $i$-th training point. The `margin` is defined as $M = min(M_1,\ldots, M_n)$. The optimal classification rule (maximal separating hyperplane) maximizes the margin.] The optimal classification rule is the line that *maximizes the margin* around the separating line.  Such a classifier is called *the maximal margin classifier*. Formally, we consider the optimization problem:
$$
\max M \;\; \hbox{ subject to } \left\{ \begin{matrix}Y_i(\beta_0 + X_{i1}\beta_1 + \ldots +  X_{ip}\beta_p) \geq M, \; i = 1, \ldots, n, \\ \beta_1^2 + \ldots + \beta_p^2 = 1, \end{matrix}\right.
$$
with respect to $\beta_0, \ldots, \beta_p$. The two conditions ensure that, when $M > 0$, each observation will remain on the correct side of the boundary and is at least distance $M$ from the boundary. We seek the largest such $M$ and associated parameters. 

It can be shown that the optimization problem above is equivalent to the following optimization problem:
$$
\min \;  (\beta_1^2 + \ldots + \beta_p^2)  \;\; \hbox{ subject to } Y_i(\beta_0 + X_{i1}\beta_1 + \ldots +  X_{ip}\beta_p) \geq 1, \;  i = 1, \ldots, n,
$$
with respect to $\beta_0, \ldots, \beta_p$. To understand the geometry behind the optimization problem above, we note that for perfectly separable data, $Y_i(\beta_0 + X_{i1}\beta_1 + \ldots +  X_{ip}\beta_{ip}) > 0$ for each $i$. The constraint $Y_i(\beta_0 + X_{i1}\beta_1 + \ldots +  X_{ip}\beta_p) \geq 1$ implies $\beta_0 + X_{i1}\beta_1 + \ldots +  X_{ip}\beta_p \geq 1$ if $Y_i = 1$, and that $\beta_0 + X_{i1}\beta_1 + \ldots +  X_{ip}\beta_p \leq -1$ if $Y_i = -1$.  Thus the constraints above define an empty slab or margin around the linear decision boundary of thickness $1/(\beta_1^2 + \ldots + \beta_p^2)^{1/2}$. Hence we choose $\beta_0, \beta_1, \ldots, \beta_p$ to maximize its thickness, or equivalently minimize $(\beta_1^2 + \ldots + \beta_p^2)$. The boundaries of this empty slab/margin are determined by the points that exactly satisfies the condition
$$
\beta_0 + X_1\beta_1 + \ldots +  X_p\beta_p = \pm 1. 
$$

Once we obtain the estimators $\widehat\beta_0, \widehat\beta_1, \ldots, \widehat\beta_p$, the optimal separating hyperplane is $\widehat f(\x) = \widehat\beta_0 + x_1 \widehat\beta_1 + \ldots + x_p\widehat\beta_p$. Thus our classification rule is as follows: for a new observation $\x = (x_1, \ldots, x_p)$, 
$$
\widehat Y = sign\{\widehat f(\x)\}.
$$

```{r svm2, echo=F, fig.height=6, fig.width=6, fig.margin=TRUE, fig.cap="Simulated two class data with support vectors and separating hyperplane (line) highlighted."}
...
$$
Y_i(\beta_0 + X_{i1}\beta_1 + \ldots +  X_{ip}\beta_p) = 1,
$$
...
```{r svmns, echo=F, fig.height=6, fig.width=6, fig.margin=TRUE, fig.cap="Simulated two-class data with separating hyperplane (line) highlighted for the non-separable case."}
library(kernlab)
set.seed(1)
n <- 10
div <- 0.5
...
When the two classes are not linearly separable (e.g., Figure \ref{fig:svmns}), we will not be able to find a line that entirely separates the groups, that is, the maximal margin classifier can not be computed. Thus, the two classes cannot be classified exactly. We can, however, generalize the ideas to develop a classification rule that *almost* separates the classes. To do so, we allow a few points to fall on the wrong side of the margin or separating hyperplane. Such a classifier is called a *support vector classifier* or a *soft-margin classifier*.^[The term "soft margin" is coined because the margin can be violated by some of the training observations.] 

Even if the classification problem is completely separable, we still may consider such soft margin classifier for robustness. A separating hyperplane based classifier will perfectly classify all of the training data (training error = 0), which can lead to sensitivity to individual observations. An example
...
knitr::include_graphics("Hastie_ISLR2_figures/Chapter9/9_5.pdf")
```
