...
- We have $`r exp(cc[1])`$ odds of heart disease vs not having heart disease. 
- Of course, zero values for `Age` and `RestingBP` doesn't make sense!
...
- Equivalently, our odds will change by a multiplicative factor of $`r exp(cc[2])`$ (in other words, increase by $`r round(100*(exp(cc[2]) - 1), 3)`$ percent). 
...
```{r}
# Confusion matrix
confusionMatrix(table(heart_data$HeartDiseaseFac, 
                         predicted = ifelse(predict(heart_glm, type = "response") > 0.5,
                                            1, 0)
))
```
...
#               col = "white") +  
  geom_point(aes(Age, RestingBP,
                 col = HeartDiseaseFac, pch = HeartDiseaseFac), size = 2,
             data = heart_data |> dplyr::filter(Sex == "F")) +
  labs(fill = "P(Y = 1 | Age, RestingBP, Female)", 
       x = "Age",
       y = "RestingBP") +
  scale_fill_viridis_c(option = "B", guide = ) + 
  theme(legend.position = "top", legend.text = element_text(angle = 45)) +
...
```{r}
confusionMatrix(table(heart_data |> 
                        filter(Sex == "F") |> 
                        pull(HeartDiseaseFac), 
                      predicted = ifelse(predict(heart_glm, 
...
#               col = "white") +  
  ...
  labs(fill = "P(Y = 1 | Age, RestingBP, Male)", 
       x = "Age",
       y = "RestingBP") +
  scale_fill_viridis_c(option = "B", begin = 0, end = 1, guide = ) + 
  theme(legend.position = "top", legend.text = element_text(angle = 45)) + 
  geom_abline(slope = -cc[2]/cc[3], 
