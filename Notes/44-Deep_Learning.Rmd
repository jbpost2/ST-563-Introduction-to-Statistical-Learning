...
ytrain <- y[-ind]
```
\noindent Notice that we have used `%>%` (pipe) operator here which is similar to the base R pipe `|>`.
...
```{r nnhist, fig.cap="Training and test set metrics over epochs. The loss for the training and validation sets acts very similalry, starting high and dropping down near 0 after about 50 epochs. The validation error is slightly higher until this time but then acts very similarly to the training error. THe mean absolute error for the training and test sets both start high and decrease. The training error is lower than that of the validation error throught most of the process, including as the epochs grow to 200 - the largest value on the plot.", message=FALSE, warning=FALSE, echo=FALSE}
