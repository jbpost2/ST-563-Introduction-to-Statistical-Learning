...
- Let's look at the relationships between our wine `Class` and some of the numeric variables in the data
...
```{r, fig.width=8, fig.height=5, echo=FALSE, fig.cap="Estimated distribution of Alcohol for the two groups.", fig.margin=T}
# set up the grid over which to plot
Alcohol <- seq(10, 16, length.out = 101)
...
- This classification method is called **Quadratic Discriminant Analysis (QDA)**. 
    ...
    + Let's see the decision boundary here.
...
```{r, fig.width=8, fig.height=5, echo=FALSE, fig.cap="Estimated distribution of Alcohol for the two groups.", fig.margin=T}
first2_alc <- 10
first3 <- min(which(preds==3))
first3_alc <- Alcohol[first3]
first1 <- min(which(preds==1))
first1_alc <- Alcohol[first1]
last1_alc <- 16
# Normal pdfs of two groups
f1 <- dnorm(Alcohol, mean = xbar[1], sd = sdev[1])
f2 <- dnorm(Alcohol, mean = xbar[2], sd = sdev[2])
...
ggplot(gg) + 
  ...
  #labs(col=guide_legend(title="Class")) + 
  scale_color_discrete(labels = 1:3) +
  geom_segment(x = first2_alc, xend = Alcohol[first3-1], y = 0, yend = 0, lwd =2, col = "#00BA38")+
  geom_segment(x = first3_alc, xend = Alcohol[first1-1], y = 0, yend = 0, lwd =2, col = "#619CFF")+
  geom_segment(x = first1_alc, xend = last1_alc, y = 0, yend = 0, lwd =2, col = "#F8766D")
```
...
The plots below give an example of this joint distribution and an example of a random sample from it.
...
- An easy was to visualize the PDF of a bivariate distribution is to plot the constant probability density contours.

**Constant probability density contours**

We define the constant probability density contour (also called constant-density contour) of a bivariate normal PDF to be the set of vectors $\boldsymbol{x}$ such that $f(\boldsymbol{x})$ is constant, These sets are ellipses that are centered around $\boldsymbol{\mu}$.

The figures below show three examples of bivariate normal distribution with different variance-covariance patterns. 

Note:

- If the two variables are uncorrelated (covariance is 0), the major and minor axes of the elliptical contours will be parallel to the $x$- and $y$-axis. 
- In presence of correlation, the ellipses will be oriented according the sign/magnitude of the correlation. 
...
- As before, an equivalent classification rule can be constructed using $\widehat{\delta}_k(x)$: 
...
knitr::include_graphics("img/4_9.png")
```

The figure above shows examples of simulated data sets -- figure taken from *Introduction to Statistical Learning*. 

- Each of the data sets contains two variables $X_1$ and $X_2$ and two classes. 
- The left panel shows the data with $cor(X_1, X_2) = 0.7$ for both the classes. Thus the Bayes decision boundary is linear in this case, and LDA performs better than QDA, because QDA suffers from higher variance without a corresponding decrease in bias.   
- The right panel shows a data set with $cor(X_1, X_2) = 0.7$ in one class and $cor(X_1, X_2) = -0.7$ in the other. Here, the assumption of a common variance-covariance matrix is not appropriate and thus, LDA suffers from high bias but QDA performs better. 
...
$$
\widehat f_{X_j|Y}(x_j|y=k) = \frac{1}{n_{train, k}} \sum_i I(X_{ij} = x_j) 
$$
where $n_{train, k}$ is the size of the training set for the $k$-th class.
...
```{r, out.width = '400px', fig.cap="Accuracy and Kappa values for the four models on the wine data set."}
...
The textbook \textit{Introduction to Statistical Learning} also provides numerical comparison between various classifiers discussed so far. The figure below shows test error rates under different scenarios -- see the textbook for details for each scenarios.
