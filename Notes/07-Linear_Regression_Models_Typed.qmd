...
```{r example, echo=FALSE, fig.cap="Examples of functions that can be captured by appropriate linear models.", fig.width=5, fig.margin = TRUE, fig.alt = 'The image is a line graph displaying four different functions plotted on a grid. The x-axis is labeled "x," ranging from 0 to 4, and the y-axis is labeled "f(x)," ranging from -5 to 20. Four colored lines represent different functions: a red line for "BSpline," which rises steeply, dips, and then ascends sharply; a green line for "Cubic," which starts low, dips, and then rises continually; a cyan line for "Cubic," which begins high, dips to around zero, and then levels off; and a purple line for "Fractional," which runs relatively flat, slightly ascending. The graph grid consists of evenly spaced horizontal and vertical lines. The functions are labeled in the top left corner alongside their colors.'}
x <- seq(0.2,4, len=51)
df <- tibble(x = x,
             Linear = 1 + x,
...
```{r, fig.cap = "At each value of the predictor, we assume the values of the response variable are normally distributed about the line. Figure from physicsforums.com", fig.width=4, echo = FALSE, fig.alt = 'The image is a graph depicting a relationship between two variables, labeled "Predictor (X)" on the horizontal axis and "Response (Y)" on the vertical axis. The graph features a series of symmetrical, bell-shaped curves arranged horizontally across the graph. These curves are shaded in yellow and represent variability or distribution at different levels of the Predictor variable. A red line diagonally traverses the graph, indicating a trend or average relationship between the X and Y variables. The curves are evenly spaced along the red line, illustrating distribution around the trend line following a consistent normal distribution, regardless of the value of X.'}
...
$$
RSS = \sum_{i=1}^n e_i^2 = \sum_{i=1}^n (Y_i - \widehat Y_i)^2.
$$
...
```
```{r echo = FALSE}
temp_solution <- cbind(row.names(solution), solution)
row.names(temp_solution) <- NULL
temp_solution <- as.data.frame(temp_solution)
names(temp_solution) <- c("Parameter", "Estimate")
temp_solution |>
  kable()
```
...
```{r outlier, echo=FALSE, fig.cap="Example of an outlier (red point). Shown are two regression lines fit with OLS: before (red dashed) and  after (blue solid) removing the outlier.", message=FALSE, fig.width = 4, fig.alt = 'The image is a scatter plot on a Cartesian plane displaying data points, a linear regression line, and another line for comparison. The x-axis is labeled from -2 to 2, and the y-axis from -1 to 3. Black circular markers represent the individual data points, clustered mainly between the y-values of -1 and 2. A noted outlier is a red triangle located at x=2 and y=3. A blue line indicating a linear trend follows the general direction of the data points, sloping slightly upwards. This is the fitted least squares curve without the outlier included. A red dashed line runs parallel and slightly above the blue line across the graph. This indicates the least squares fit including the outlier point. Both the slope and intercept are changed by a reasonable amount by the addition of one point.'}
set.seed(1001)
...
```{r, echo = FALSE, fig.height = 3, fig.width = 3, fig.cap = "Visual of the loss associated with squared error loss and huber loss. Source: Wikipedia", fig.alt = 'The image is a graph plotting two curves on a Cartesian coordinate system. The horizontal x-axis ranges from -4 to 4, and the vertical y-axis ranges from 0 to 8. Both axes are evenly ticked. The blue curve opens upwards and represents a quadratic function, forming a U-shape that is symmetrical around the y-axis. It starts at the top left corner at (-4, 8), dips to a minimum point near (0, 0), and rises to the top right corner at (4, 8). The green curve is a straight line taking on smaller values than the blue curve except in the region where x is between -1 and 1. In that region the green curve exactly follow the parabola of the blue curve.'}
...
```{r outlier2, echo=FALSE, fig.cap="Example of an outlier (red point). Shown are two regression lines fit with absolute error loss: before (red dashed) and  after (blue solid) removing the outlier.", message=FALSE, fig.width = 4, fig.alt = 'The image is a scatter plot on a Cartesian plane displaying data points, a linear regression line, and another line for comparison. The x-axis is labeled from -2 to 2, and the y-axis from -1 to 3. Black circular markers represent the individual data points, clustered mainly between the y-values of -1 and 2. A noted outlier is a red triangle located at x=2 and y=3. A blue line indicating a linear trend follows the general direction of the data points, sloping slightly upwards. This is the fitted least squares curve without the outlier included. A red dashed line runs parallel and slightly above the blue line across the graph. In comparison to the graph of the least squares fit earlier, the slope and intercept are more similar here.'}
set.seed(1001)
df <- tibble(x = rnorm(100), 
                 y = 0.1*x + rnorm(100)/2)
dfnew <- rbind(df, c(2, 3))
...
-   The right hand side contains the regression formula.
    ...
    - We can include polynomial terms using the `poly()` function or by using `I()` and specifying the relationship. "`y ~ pred1 + I(pred1^2)`" or "`y ~ poly(pred1, 2, raw = TRUE)`"
...
```{r quad, echo=FALSE, fig.cap="Least squares fit using a quadratic term.", fig.alt = "A scatterplot between temperature on the x-axis and rented bike count on the y-axis. The plots shows a general upward trend with more spread in the rented bike count as the temperature increases. The overlayed quadratic line has a positive slope and shows a strong linear relationship. The line is nearly indistinguishable from the basic SLR fit from earlier. "}
...
- Inference can still be done though! We must be careful that the colinearity of `temperature` with `temperature^2` is accounted for (note the correlation between the two is `r round(cor(bike_share$temperature, bike_share$temperature^2), 4)`)
    ...
    - *We can use orthogonal polynomials to avoid this (which is the default of `poly()` in `R`!)*
...
- We have an interaction if the effect on `rented_bike_count`associated with `temperature` is different for lower `wind_speed`s than it is for higher `wind_speed`s
- Equivalently, this would imply a differ effect for `wind_speed` on `rented_bike_count` for lower `temperature`s as compared to higher `temperature`s
- This would make sense here as cold and windy is very different than warm and windy on how likely people are to be out and about, possibly renting bikes!
...
```{r, fig.align='center', fig.width=3, echo = FALSE, fig.alt = 'The image is a 3D scatter plot depicting data points related to bike rentals, temperature, and wind speed. On the x-axis, labeled as "temp," values range from -10 to 30. The y-axis, labeled as "bike rentals," ranges from 0 to 3000. The z-axis, labeled as "wind," ranges from 0 to 6. The plot displays numerous black dots spread across the three-dimensional space, indicating individual data points. A dark blue triangular plane represents the best saddle fit to the data, passing through the cluster of dots.', fig.cap = "Best saddle surface fit through the data."}
...
interaction* in the model
    ...
    + If the model has the term $X_1X_2$, we also retain terms $X_1$ and $X_2$!
...
The indicator variable created is called `holidayNo Holiday`. This variable takes on 1 when `holiday` takes on `No Holiday`. We can then see that our model has different intercepts but the same slope.

$$
\mbox{For Holidays: }\hat{y} = 211.6793 + 28.9532(temperature)
$$

$$
\mbox{For Non-Holidays: }\hat{y} = 211.6793 + 126.1416 + 28.9532(temperature) = 337.8209 + 28.9532(temperature)
$$

```{r echo=FALSE, fig.cap="Regression line for No Holiday (0) and Holiday (1) based on a linear model with main effects for holiday and temperature."}
...
$$
\mbox{For Holidays: }\hat{y} = 151.0245 + 35.0493(temperature)
$$

$$
\mbox{For Non-Holidays: }\hat{y} = 151.0245 + 190.3090 + 35.0493(temperature) + (-6.3656)(temperature) = 341.3335 + 28.6873(temperature)
$$

```{r echo=FALSE, fig.cap="Regression line for No Holiday (0) and Holiday (1) based on a linear model with main effects and an interaction effect for holiday and temperature."}
ggplot(bike_share, aes(temperature, rented_bike_count, color = holiday)) + 
  geom_point(alpha = 0.25, size = 1) + 
    geom_smooth(method = "lm",
              lwd = 2,
...
AmesHousing::ames_raw |>
  select(1:5) |> #grab first five columns
  slice(1:5) |>
  kable()
```
...
$$Y_i | X_i \stackrel{independent}\sim N(\beta_0 + X_i\beta_1, \sigma^2)$$
...
- We can view this number as "sample size - number of $\beta$ parameters in the mean function". 
- In SLR we have two parameters in mean function, and thus we have the term $n-2$.
- In general, with $p$ predictors, the total number of parameters in the mean function is $p + 1$ (since we need to include the intercept). 
...
```{r tcdf, echo=FALSE, fig.margin = T, fig.cap="t PDF and quantiles. The shaded region has area p, and the x-axis value corresponding to the solid vertical line represents the  (1-p)-quantile. In this example, we have p = 0.05, and the vertical line represents the upper 0.95-quantile.", fig.height=5, fig.width=6, fig.alt = 'A t distribution with three degrees of freedom is displayed. This is a distribution that is bell-shaped, centered at 0, having fatter tails than a standard normal distribution. The right tail above 2.2 or so is shaded. This shaded region represents the probability associated with a p-value.'}
...
```{r tpv, echo=FALSE, fig.margin = T, fig.cap="Two-tailed p-value for a t-test.", fig.alt = 'A t distribution with an arbitrary n-1 degrees of freedom is displayed. This is a distribution that is bell-shaped, centered at 0, having fatter tails than a standard normal distribution. The right tail above 2.2 or so and the left tail below -2.2 or so are both shaded. This shaded region represents the probability associated with a two-tailed p-value.'}
...
summary(boston_interaction)$coefficients |>
  kable()
```

- Note that the interaction term is statistically significant
but the *main effect of age* is not significant. 
- As the significant interaction implies an effect of `age` on `medv` that differs with respect to `lstat`, we still conclude that `age` is important!
- We also do not want to remove the `age` main effect for interpretability purposes.
...
$$
- We reject $H_0$ if the observed value of $F$ is "large
enough" or we calculate a p-value = $P(F_{q, n-p-1} > F_{obs})$
...
```{r}
summary(MLR_binary_interaction)$coefficients |>
  kable()
summary(MLR_binary_interaction)$fstatistic |>
  kable()
```

Note the large F-stat value. This corresponds to a p-value very close to 0.
...
- Typically, we set $\alpha = 0.05$ as level of the tests. 
    ...
    + That means, for individual $t$-tests, there is a $5\%$ chance that some effect will be detected as significant when it is actually not significant. 
    + Looking at individual $t$-tests, we should on average have about $0.05*p$ many false significant results, just by
chance, even if none of the predictors might be useful. 
    + In fact, for large $p$, it is very likely we will observe at least one false significance just by chance!
...
```{r}
summary(MLR_log_int)$coefficients
```
